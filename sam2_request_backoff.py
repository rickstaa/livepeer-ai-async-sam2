...
# Configure retries with backoff factor for delays
retries = Retry(
    total=3,  # Total number of retries
    backoff_factor=2,  # Delay factor (e.g., 1, 2, 4, 8 seconds)
    status_forcelist=[503],  # Retry on these HTTP status codes
    raise_on_status=False  # Do not raise an exception on status codes
)
